  ...
  <div class="app-container calendar-list-container">
    <basic-container>
      <div class="header-search">
        <div class="lside">
          <el-form :inline="true">
            <el-form-item :label="$t('upgradeInfo.versionNumber')">
              <el-input v-model="listQuery.versionCode" :placeholder="$t('common.inputPrompt')"></el-input>
            </el-form-item>
        ...
        <div class="rside">
          <el-button type="primary" plain @click="handleForm()" v-if="permissions.admin_lsyupgrade_add">{{$t('common.add')}}</el-button>
        </div>
      </div>
      ...
      <div class="avue-crud">
        <el-table :data="dataList" fit v-loading="listLoading">
          ...
          <el-table-column align="center" :label="$t('upgradeInfo.updateTime')" min-width="150">
            <template slot-scope="scope">
              {{ scope.row.updateTime | parseTime }}
            </template>
          </el-table-column>
          <el-table-column align="center" :label="$t('common.createTime')" min-width="150">
            <template slot-scope="scope">
              {{ scope.row.createTime | parseTime }}
            </template>
          </el-table-column>
          <el-table-column :label="$t('common.operate')" align="center" min-width="150">
            <template slot-scope="scope">
              <el-button type="text" :disabled="scope.row.status === 1" @click="handleSend(scope.row.id)" v-if="permissions.admin_lsyupgrade_issued">下发</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
export default {
  ...
  methods: {
    getData(state) {
      ...
      qryUpdate(this.listQuery).then(res => {
