<template>
  <div>
      <div class="header-search">
        <div class="lside">
          <el-form :inline="true">
            <el-form-item :label="$t('faultInfo.equipmentCode')">
              <el-input v-model="listQuery.sn" :placeholder="$t('common.inputPrompt')"></el-input>
            </el-form-item>
            <el-form-item :label="$t('faultInfo.faultCode')">
              <el-input v-model="listQuery.faultCode" :placeholder="$t('common.inputPrompt')"></el-input>
            </el-form-item>
      ...
      <div class="avue-crud">
        <el-table :data="dataList" border fit v-loading="listLoading">
          ...
          <el-table-column align="center" :label="$t('faultInfo.processingStatus')">
            <template slot-scope="scope">
              <span v-if="scope.row.handleStatus === 0">{{$t('faultInfo.occur')}}</span>
...
export default {
  ...
  methods: {
    getData(state) {
      ...
      qryDeviceAlarm(this.listQuery).then(res => {
