<template>
  <el-dialog :title="$t('userManage.userAddEdit')" :visible.sync="visible" width="520px" :close-on-click-modal="false">
    <el-form ref="dataForm" :model="dataForm" label-width="120px" :rules="rules">
      <el-form-item :label="$t('common.loginAccount')" prop="username">
        <el-input v-model="dataForm.username" :placeholder="$t('common.loginAccount')" maxlength="20" />
      </el-form-item>
      <el-form-item :label="$t('common.userName')" prop="name">
        <el-input v-model="dataForm.name" :placeholder="$t('common.userName')" maxlength="10" />
      </el-form-item>
      <!-- <el-form-item label="手机号" prop="phone">
        <el-input v-model="dataForm.phone" placeholder="手机号" maxlength="11" />
      </el-form-item> -->
      <el-form-item :label="$t('common.loginPassword')" prop="password" v-if="!dataForm.userId">
        <el-input v-model="dataForm.password" :placeholder="$t('common.loginPassword')" type="password" />
      </el-form-item>
      ...
      <el-form-item :label="$t('userManage.status')" prop="lockFlag">
        <el-radio v-model="dataForm.lockFlag" :label="0">{{$t('common.enable')}}</el-radio>
        <el-radio v-model="dataForm.lockFlag" :label="9">{{$t('common.disable')}}</el-radio>
      </el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      dataForm: {},
      rules: {
        username: [{ required: true, message: this.$t('userManage.validateAccount'), trigger: "blur" }],
        name: [{ required: true, message: this.$t('userManage.validateName'), trigger: "blur" }],
        password: [{ required: true, message: this.$t('userManage.validatePassword'), trigger: "blur" }],
        roleId: [{ required: true, message: this.$t('userManage.validateRole'), trigger: "blur" }],
