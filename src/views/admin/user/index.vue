  ...
  <div class="app-container calendar-list-container">
    <basic-container>
      <div class="header-search">
        ...
        <div class="rside">
          <el-button type="primary" plain @click="handledAddorEdit()" v-if="permissions.sys_user_add">{{$t('common.add')}}</el-button>
        </div>
      </div>

      <div class="avue-crud">
        <el-table :data="dataList" border v-loading="listLoading">
          <el-table-column align="center" :label="$t('common.loginAccount')" prop="username"/>
          ...
          <el-table-column align="center" :label="$t('userManage.status')">
            <template slot-scope="scope">
              <el-tag v-if="scope.row.lockFlag === '0'">{{$t('userManage.normal')}}</el-tag>
              <el-tag type="danger" v-else-if="scope.row.lockFlag === '9'">{{$t('userManage.lock')}}</el-tag>
            </template>
          </el-table-column>
          <el-table-column align="center" :label="$t('userManage.operate')" width="280">
            <template slot-scope="scope">
              <div v-if="scope.row.username !== 'admin'">
                <el-button type="text" v-if="permissions.sys_user_lock" @click="handleNormal(scope.row.userId)">{{ scope.row.lockFlag == '0' ? $t('common.disable') : $t('common.restore') }}</el-button>
                <el-button type="text" v-if="permissions.sys_user_edit" @click="handledAddorEdit(scope.row)">{{$t('common.edit')}}</el-button>
                <el-button type="text" v-if="permissions.sys_user_password_reset" @click="handleResetPwd(scope.row.userId)">{{$t('common.resetPassword')}}</el-button>
                <el-button type="text" v-if="permissions.sys_user_del" style="color: red;" @click="deleteHandle(scope.row.userId)">{{$t('common.delete')}}</el-button>
              </div>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    // 删除
    deleteHandle (id) {
      this.$confirm(this.$t('userManage.sureDelete'), this.$t('common.prompt'), {
