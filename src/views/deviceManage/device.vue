  ...
  <div class="app-container calendar-list-container">
    <basic-container v-show="pageState === 1">
      <div class="header-search">
        <div class="lside">
          <el-form :inline="true">
            ...
            <el-form-item :label="$t('deviceManage.snCode')">
              <el-input v-model="listQuery.sn" :placeholder="$t('common.inputPrompt')"></el-input>
            </el-form-item>
            <el-form-item :label="$t('deviceManage.serialNumber')">
              <el-input v-model="listQuery.serialNumber" :placeholder="$t('common.inputPrompt')"></el-input>
            </el-form-item>
            <el-form-item :label="$t('deviceManage.onlineStatus')">
              <el-select v-model="listQuery.onlineStatus" :placeholder="$t('common.selectPrompt')">
                <el-option :label="$t('deviceManage.offline')" :value="0"></el-option>
                <el-option :label="$t('deviceManage.online')" :value="1"></el-option>
        ...
        <div class="rside">
          <el-button type="primary" plain @click="handleForm()" v-if="permissions.admin_lsydevice_add">{{$t('common.add')}}</el-button>
        </div>
      </div>

      <div class="avue-crud">
        <el-table :data="dataList" fit v-loading="listLoading">
          <el-table-column align="center" prop="name" :label="$t('deviceManage.deviceName')" />
          <el-table-column :label="$t('deviceManage.snCode')" align="center">
            <template slot-scope="scope">
              <el-link type="primary" :underline="false" @click="handleLook(scope.row.sn, scope.row.sn)">{{ scope.row.sn }}</el-link>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="serialNumber" :label="$t('deviceManage.serialNumber')" />
    ...
    <!-- 详情 -->
    <DeviceStatistics v-if="pageState === 2" ref="deviceStatistics" @back="pageState = 1" />

    <!-- 运行信息 -->
    <RealTimeData v-if="pageState === 3" ref="realTimeData" @back="pageState = 1" />
...
export default {
  ...
  methods: {
    getData(state) {
      ...
      qryDevice(this.listQuery).then(res => {
