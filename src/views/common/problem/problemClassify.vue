  ...
  <div class="app-container calendar-list-container">
    <basic-container>
      <div class="header-search">
        <div class="lside">
          <el-form :inline="true">
            <el-form-item :label="$t('classifyManage.classificationName')">
      ...
      <div class="avue-crud">
        <el-table :data="dataList" border fit v-loading="listLoading">
          <el-table-column align="center" :label="$t('classifyManage.classificationNameC')" prop="title" />
          <el-table-column align="center" :label="$t('classifyManage.classificationNameE')" prop="titleEn" />
          <el-table-column align="center" :label="$t('classifyManage.description')" prop="sketch" />
    ...
    <ClassifyForm ref="classifyForm" @back="getData" />
  </div>
</template>
<script>
import { qryProblemClassify, delProblemClassify } from '@/api/common/problem'
import { mapGetters } from "vuex"
import ClassifyForm from './components/classifyForm.vue'
export default {
  components: { ClassifyForm },
  data () {
    return {
      // 列表信息
      listLoading: false,
  ...
  methods: {
    getData(state) {
      ...
      qryProblemClassify(this.listQuery).then(res => {
    ...
    handleAddorEdit(info) {
      this.$refs.classifyForm.init(info)
    },
     // 每页数
