    ...
    <basic-container>
          ...
          <el-form :inline="true">
            <el-form-item :label="$t('versionManage.type')">
              ...
              >
                ...
                />
                <el-option
                  :label="$t('versionManage.inverterMCU')"
                  :value="3"
                />
                <el-option
                  :label="$t('versionManage.inverterDSP')"
                  :value="4"
                />
                ...
                <el-option
                  :label="$t('versionManage.HighVoltageBatteryBCUModule')"
                  :value="9"
                />
                <el-option
      ...
      <div class="avue-crud">
        <el-table :data="dataList" border fit v-loading="listLoading">
          <el-table-column
            :label="$t('versionManage.type')"
            align="center"
            width="200"
          >
            <template slot-scope="scope">
              <span>{{ appTypeFilter[scope.row.appType] }}</span>
              <span v-if="scope.row.appType < 2"
                >-{{
                  scope.row.appItem === 1
                    ? $t("common.user")
          ...
          <el-table-column
            :label="$t('versionManage.download')"
            align="center"
            prop="adminComFileInfosVo.filePath"
            min-width="150"
          />
          <el-table-column
      ...
      >
        ...
        >
          <el-form-item :label="$t('versionManage.type')">
            ...
            <el-radio v-model="dataForm.appType" :label="3">{{
              $t("versionManage.inverterMCU")
            }}</el-radio>
            <el-radio v-model="dataForm.appType" :label="4">{{
              $t("versionManage.inverterDSP")
            }}</el-radio>
            <el-radio v-model="dataForm.appType" :label="5">{{
              $t("versionManage.battery")
            }}</el-radio>

            <el-radio v-model="dataForm.appType" :label="6">{{
              $t("versionManage.IIcb")
            }}</el-radio>
            <el-radio v-model="dataForm.appType" :label="7">{{
              $t("versionManage.IIdb")
            }}</el-radio>
            <el-radio v-model="dataForm.appType" :label="8">{{
              $t("versionManage.IIbattery")
            }}</el-radio>

            <el-radio v-model="dataForm.appType" :label="9">{{
              $t("versionManage.HighVoltageBatteryBCUModule")
            }}</el-radio>
          ...
          <el-form-item
            ...
            v-if="dataForm.appType < 2"
          >
            <el-radio v-model="dataForm.appItem" :label="1">{{
              $t("common.user")
            }}</el-radio>
          ...
          <el-form-item
            :label="$t('versionManage.file')"
            prop="adminComFileInfosAddDto.fileId"
          >
            <el-upload
              class="upload-demo"
              drag
            ...
            >
              ...
              <div class="el-upload__text">
                {{ $t("versionManage.upload1") }}
                <em>{{ $t("versionManage.upload2") }}</em>
              </div>
            </el-upload>
...
import {
  ...
  AddVersioninfo,
  delVersioninfo,
} from "@/api/common/versionUpgrade";
...
export default {
  ...
  data() {
    return {
      ...
      dataForm: {},
      dataRule: {
        "adminComFileInfosAddDto.fileId": [
          {
            required: true,
            message: this.$t("versionManage.validateFile"),
            trigger: "blur",
          },
        ],
        versionCode: [
          {
            required: true,
            message: this.$t("versionManage.validateCode"),
            trigger: "blur",
          },
        ],
        versionName: [
          {
            required: true,
            message: this.$t("versionManage.validateName"),
            trigger: "blur",
          },
        ],
      },
      cabinetId: null,
      appTypeFilter: [
        this.$t("versionManage.android"),
        this.$t("versionManage.ios"),
        this.$t("versionManage.networkConnectingBoard"),
        this.$t("versionManage.inverterMCU"),
        this.$t("versionManage.inverterDSP"),
        this.$t("versionManage.battery"),
        this.$t("versionManage.IIcb"),
        this.$t("versionManage.IIdb"),
  ...
  methods: {
    ...
    handleDel(id) {
      this.$confirm(
        this.$t("versionManage.sureDelete"),
